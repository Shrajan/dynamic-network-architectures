...
from dynamic_network_architectures.building_blocks.dws_conv_blocks import DWS_ConvDropoutNormReLU
from dynamic_network_architectures.building_blocks.regularization import DropPath, SqueezeExcite
import numpy as np


class DWS_BasicBlockD(nn.Module):
    def __init__(self,
        ...
        self.conv1 = DWS_ConvDropoutNormReLU(conv_op, input_channels, output_channels, kernel_size, stride, conv_bias,
                                         norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)
        self.conv2 = DWS_ConvDropoutNormReLU(conv_op, output_channels, output_channels, kernel_size, 1, conv_bias, norm_op,
        ...
        if has_stride or requires_projection:
            ...
            if requires_projection:
                ops.append(
                    DWS_ConvDropoutNormReLU(conv_op, input_channels, output_channels, 1, 1, False, norm_op,
                                        norm_op_kwargs, None, None, None, None
                                        )
                )
...
class DWS_BottleneckD(nn.Module):
    def __init__(self,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
        ...
        self.conv1 = DWS_ConvDropoutNormReLU(conv_op, input_channels, bottleneck_channels, 1, 1, conv_bias,
                                         norm_op, norm_op_kwargs, None, None, nonlin, nonlin_kwargs)
        self.conv2 = DWS_ConvDropoutNormReLU(conv_op, bottleneck_channels, bottleneck_channels, kernel_size, stride,
                                         conv_bias,
                                         norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)
        self.conv3 = DWS_ConvDropoutNormReLU(conv_op, bottleneck_channels, output_channels, 1, 1, conv_bias, norm_op,
                                         norm_op_kwargs, None, None, None, None)
        ...
        if has_stride or requires_projection:
            ...
            if requires_projection:
                ops.append(
                    DWS_ConvDropoutNormReLU(conv_op, input_channels, output_channels, 1, 1, False,
...
class DWS_StackedResidualBlocks(nn.Module):
    def __init__(self,
                 n_blocks: int,
                 conv_op: Type[_ConvNd],
                 input_channels: int,
        ...
        assert block in [DWS_BasicBlockD, DWS_BottleneckD], 'block must be DWS_BasicBlockD or DWS_BottleneckD'
...
if __name__ == '__main__':
    ...
    stx = DWS_StackedResidualBlocks(2, nn.Conv2d, 24, (16, 16), (3, 3), (1, 2),
                                                norm_op=nn.BatchNorm2d, nonlin=nn.ReLU, nonlin_kwargs={'inplace': True},
                                                block=DWS_BottleneckD, bottleneck_channels=3)
    model = nn.Sequential(DWS_ConvDropoutNormReLU(nn.Conv2d,
